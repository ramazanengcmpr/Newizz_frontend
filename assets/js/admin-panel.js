...
class AdminPanel {
    ...
    async loadUsers() {
        ...
        try {
                    ...
                    <tbody>
                        ${users.map(u => `
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-outline-danger" onclick="adminPanel.deleteUser('${u._id}')">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </td>
                            </tr>
            ...
            loading.style.display = 'none';
            container.innerHTML = '<p class="text-center text-danger">Users cannot be loaded. Check your permissions.</p>';
        }
    }
    ...
    async deleteUser(id) {
        ...
        try {
            const result = await this.apiCall(`/auth/users/${id}`, { method: 'DELETE' });
            if (result) {
                this.showAlert('User başarıyla silindi!', 'success');
                this.loadUsers();
            }
        } catch (error) {
            console.error('User silme hatası:', error);
            this.showAlert('User silinirken hata oluştu.', 'danger');
        }
    }
    ...
    async addProperty() {
        ...
        const requiredFields = ['title', 'price', 'location', 'propertyType', 'bedrooms', 'bathrooms', 'size', 'mainImage', 'description'];
        const missing = requiredFields.filter(f => !data[f] || data[f] === '');
        if (missing.length > 0) {
            this.showAlert(`Lütfen zorunlu alanları doldurun: ${missing.join(', ')}`, 'warning');
            return;
        }
        ...
        // Helpers to parse plain text or JSON inputs
        const isJsonLike = (str) => str && (str.trim().startsWith('[') || str.trim().startsWith('{'));
        const splitLine = (line) => {
            // Prefer pipe as delimiter, fallback to comma
        ...
        const parseLines = (text) => text
            ...
            .map(l => l.trim())
            .filter(l => l.length > 0);
        const parseFloorPlansText = (text) => {
            const lines = parseLines(text);
            const items = [];
            for (const line of lines) {
                const parts = splitLine(line).map(p => p.trim());
                if (parts.length >= 2 && parts[0] && parts[1]) {
                    items.push({ title: parts[0], image: parts[1], description: parts[2] || '' });
                }
            }
            return items;
        };
        const parseNearbyText = (text) => {
            const lines = parseLines(text);
            const items = [];
            for (const line of lines) {
                const parts = splitLine(line).map(p => p.trim());
                if (parts.length >= 3 && parts[0] && parts[1] && parts[2]) {
                    items.push({ name: parts[0], distance: parts[1], type: parts[2] });
                }
            }
            return items;
        };
        ...
        // Floor Plans (accept JSON or plain text: Title | Image | Description)
        if (!data.floorPlans || data.floorPlans.trim() === '') {
            delete data.floorPlans;
        } else if (isJsonLike(data.floorPlans)) {
            try {
                const parsed = JSON.parse(data.floorPlans);
                data.floorPlans = Array.isArray(parsed) ? parsed : [parsed];
            } catch (e) {
                console.error('Floor plans JSON parse error:', e);
                delete data.floorPlans;
            }
        } else {
            const items = parseFloorPlansText(data.floorPlans);
            if (items.length > 0) {
                data.floorPlans = items;
            } else {
                delete data.floorPlans;
            }
        }

        // Nearby Education (JSON or plain text: Name | Distance | Type)
        if (data.nearbyEducation && data.nearbyEducation.trim() !== '') {
            data.nearby = data.nearby || {};
            if (isJsonLike(data.nearbyEducation)) {
                try {
                ...
                const items = parseNearbyText(data.nearbyEducation);
                if (items.length > 0) data.nearby.education = items;
            }
        ...
        if (data.nearbyStores && data.nearbyStores.trim() !== '') {
            data.nearby = data.nearby || {};
            if (isJsonLike(data.nearbyStores)) {
                try {
                ...
                const items = parseNearbyText(data.nearbyStores);
                if (items.length > 0) data.nearby.stores = items;
            }
        ...
        if (data.nearbyHealth && data.nearbyHealth.trim() !== '') {
            data.nearby = data.nearby || {};
            if (isJsonLike(data.nearbyHealth)) {
                try {
                ...
                const items = parseNearbyText(data.nearbyHealth);
                if (items.length > 0) data.nearby.health = items;
            }
